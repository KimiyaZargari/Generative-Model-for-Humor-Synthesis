...
"""
inference_single_task_endstop_compare_instruct_multilang.py
...
- --lang {en, es, zh}
- FINETUNED prompts match training:
  - conan: headline/setup + SEP
  - keywords:
      en: "<TASK_KEYWORDS>\n{k1} | {k2}" + SEP   (backward compatible)
      es: "<TASK_KEYWORDS_ES>\n{k1} | {k2}" + SEP
      zh: "<TASK_KEYWORDS_ZH>\n{k1} | {k2}" + SEP
- Output is cut at END sentinel for finetuned slot.

COMPARE MODE (INSTRUCT vs FINETUNED):
- Compares your LoRA-adapted base model against an INSTRUCT model
- Uses chat template if available for the instruct tokenizer

Examples:
  # Spanish finetuned, interactive
  python inference_single_task_endstop_compare_instruct_multilang.py \
    --task conan --lang es --lora_dir runs/latest_conan_es/lora --interactive

  # Chinese finetuned + compare vs instruct
  python inference_single_task_endstop_compare_instruct_multilang.py \
    --task conan --lang zh --compare --lora_dir runs/latest_conan_zh/lora \
    --instruct_model mistralai/Mistral-7B-Instruct-v0.3 --interactive

  # Spanish keywords, base-only
  python inference_single_task_endstop_compare_instruct_multilang.py \
    --task keywords --lang es --no_lora --interactive

Notes:
- For zh keywords: you can pass any 1–3 chars as "keywords" (or words if you want).
- For conan: just type the setup sentence in the target language.
"""
...
def build_prompt_keywords_ft(k1: str, k2: str, lang: str) -> str:
    ...
    # Backward compatibility for older EN runs that used <TASK_KEYWORDS> without language suffix.
    if lang == "en":
        tag = "<TASK_KEYWORDS>"
    elif lang == "es":
...
# ----------------------------
# INSTRUCT PROMPTS (for compare model) - multilingual
# ----------------------------
...
def run_once_finetuned(task: str, lang: str, model, tokenizer, args, k1=None, k2=None, headline=None) -> str:
    ...
    return generate_text(
        ...
        top_p=args.top_p,
        seed=args.seed,
        cut_at_end=True,  # finetuned uses END sentinel
    )
